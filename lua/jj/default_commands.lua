...
local DEFAULT_COMMANDS = {
  ...
  rebase = {
    ...
    menu = {
      ...
      options = {
        {
          key = "1",
          desc = "Rebase onto commit",
          cmd = "rebase", 
          args = {"-d", "{commit_id}"}
        },
        {
          key = "2",
          desc = "Rebase with conflicts allowed",
          cmd = "rebase",
          args = {"-d", "{commit_id}", "--allow-conflicts"}
        },
        {
          key = "3", 
          desc = "Rebase all descendants",
          cmd = "rebase",
          args = {"-d", "{commit_id}", "-s", "{change_id}"}
        }
      }
    }
  },
  ...
  abandon = {
    ...
    menu = {
      ...
      options = {
        ...
        {
          key = "2",
          desc = "Abandon with descendants", 
          cmd = "abandon",
          args = {"{change_id}", "--recursive"},
          confirm = true
        }
      }
    }
  ...
  squash = {
    quick_action = {
      ...
      args = {"--into", "{commit_id}"},
      keymap = "s",
      description = "Squash current change into selected commit",
      confirm = true
    },
    menu = {
      keymap = "S",
...
function M.execute_with_confirmation(command_name, context)
  ...
  for _, arg in ipairs(args) do
    ...
    elseif arg == "{change_id}" then  
      table.insert(substituted_args, context.change_id or context.commit_id or "@")
