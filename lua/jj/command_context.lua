...
-- Substitute selection values into command argument templates
function M.substitute_selections(args, selections)
  ...
  for _, arg in ipairs(args) do
    if type(arg) == "string" then
      ...
      -- Replace selection templates with actual values
...
function M.register_default_commands_with_phases()
  ...
  M.register_command("abandon_multiple", {
    quick_action = {
      ...
      confirm = true,
      phases = {
        { key = "targets", prompt = "Select commits to abandon (Enter to add, 'c' to confirm)", multi_select = true }
      }
    },
    menu = {
      keymap = "A",
