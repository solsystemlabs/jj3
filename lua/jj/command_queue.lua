...
function M.process_queue()
  ...
  for _, queued_command in ipairs(queue_state.commands) do
    ...
    if success then
      ...
      vim.notify("jj.nvim: Queued command failed: " .. queued_command.command .. " - " .. (error_msg or "unknown error"), vim.log.levels.ERROR)
    end
  end
  ...
  if failed_count > 0 then
    vim.notify("jj.nvim: Queue processed: " .. successful_count .. " succeeded, " .. failed_count .. " failed", vim.log.levels.WARN)
  else
