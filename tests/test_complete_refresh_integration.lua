...
describe("Complete Refresh System Integration", function()
  ...
  describe("full workflow integration", function()
    it("should handle complete open -> navigate -> refresh -> navigate workflow", function()
      ...
      local window_mock = {
        ...
        refresh_log_content = function() return true end,
        refresh_navigation = function() return true end
      }
      
      local refresh_success = refresh.manual_refresh(window_mock)
      assert.is_true(refresh_success)
      
      -- Should have refresh notifications
      local has_refresh_message = false
      for _, notification in ipairs(mock_notifications) do
        if notification.message:match("Refreshing jj log") then
          has_refresh_message = true
          break
        end
      end
      assert.is_true(has_refresh_message)
  ...
  describe("error handling integration", function()
    it("should handle refresh failures gracefully without breaking navigation", function()
      ...
      -- Mock failed refresh
      local window_mock = {
        is_log_window_open = function() return false end -- Simulate window closure
      }
